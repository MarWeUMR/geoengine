...
mod tests {
    ...
    use crate::{
        ...
        source::{
            ...
            GdalMetaDataRegular, GdalSource, GdalSourceParameters, GdalSourceTimePlaceholder,
            TimeReference,
        },
    };
    ...
    use geoengine_datatypes::primitives::{
        BoundingBox2D, Coordinate2D, DateTimeParseFormat, QueryRectangle, SpatialPartition2D,
    };
    ...
    use geoengine_datatypes::{
        dataset::DatasetId,
        hashmap,
        primitives::{Measurement, SpatialResolution, TimeGranularity, TimeInterval, TimeStep},
    ...
    fn get_gdal_config_metadata(path: &str, content_type: ContentType) -> GdalMetaDataRegular {
        let no_data_value = Some(-1000.0);

        let query_bbox =
            SpatialPartition2D::new((474112.0, 5646336.0).into(), (522752.0, 5612032.0).into())
                .unwrap();

        let resolution = SpatialResolution::new(10.0, 10.0).unwrap();

        let time = TimeInterval::new(0, 1).unwrap();
        ...
        let gdal_config_metadata = GdalMetaDataRegular {
            ...
            // TODO: needs to be correct for target aswell!
            time_placeholders: match content_type {
                ContentType::input_data => hashmap! {
                    "%_START_TIME_%".to_string() => GdalSourceTimePlaceholder {
                    format: DateTimeParseFormat::custom("%Y-%m-%d".to_string()),
                    reference: TimeReference::Start,
                }},
                ContentType::target_data => hashmap! {
                "%_START_TIME_%".to_string() => GdalSourceTimePlaceholder {
                    format: DateTimeParseFormat::custom("%Y".to_string()),
                    reference: TimeReference::Start,
                }},
            },

            step: match content_type {
                ContentType::input_data => TimeStep {
                    granularity: TimeGranularity::Months,
                    step: 1,
                },
                ContentType::target_data => TimeStep {
                    granularity: TimeGranularity::Months,
                    step: 1,
                },
            },
    ...
    async fn get_band_data(
        ...
        let mut buffer_proc: Vec<Vec<f32>> = Vec::new();

        while let Some(processor) = stream.next().await {
            match processor.unwrap().grid_array {
                GridOrEmpty::Grid(processor) => {
                    let data = &processor.inner_grid.data;
                    // let data = &processor.data;
                    // TODO: make more generic
                    let data_mapped: Vec<f32> = data.into_iter().map(|elem| *elem as f32).collect();
    ...
    async fn workflow() {
        ...
        let paths = [
            "s2_10m_de_marburg/s2_b02_10m_2020_06_01__36days_mean_de_marburg.tiff",
            "s2_10m_de_marburg/s2_b03_10m_2020_06_01__36days_mean_de_marburg.tiff",
            "s2_10m_de_marburg/s2_b04_10m_2020_06_01__36days_mean_de_marburg.tiff",
            "s2_10m_de_marburg/s2_b08_10m_2020_06_01__36days_mean_de_marburg.tiff",
            "s2_10m_de_marburg/mundialis_classification_2020_10m_de_marrburg.tiff",
        ];

        let target_path = ["s2_10m_de_marburg/mundialis_classification_2020_10m_de_marrburg.tiff"];

        // define reservoir size
        // TODO: bigger reservoir size than dataset
        // let capacity = calculate_reservoir_size(1, "mb", paths.len(), mem::size_of::<f32>(), None);
        let capacity = 1024;
        ...
        let zipped_data: Vec<Vec<Vec<f32>>> = zip_bands_to_tiles(&paths, tile_size).await;

        // extract the target band to analyze the unique classes.
        let zipped_data_target: Vec<f32> = zip_target_to_tiles(&target_path, tile_size).await;
        println!(
            "zipped data target: {:?}",
            zipped_data_target.iter().take(100).collect::<Vec<_>>()
        );
        ...
        let bst = booster_vec.last().unwrap();

        bst.save("model_new.json");

        println!("fm: {:?}", &forward_map);
        println!("bw: {:?}", &backward_map);
        println!("bw: {:?}", &true_distribution_map);
        // predict data
    ...
    async fn zip_target_to_tiles(paths: &[&str], tile_size: usize) -> Vec<f32> {
        let mut bands: Vec<Vec<Vec<f32>>> = vec![];

        // load each band given by distinct .tif files
        for path in paths.iter() {
            let gcm = get_gdal_config_metadata(path, ContentType::target_data);
        ...
        let tiles_of_zipped_bands: Vec<Vec<Vec<f32>>> = zipped_bands.collect().await;
    ...
    async fn zip_bands_to_tiles(paths: &[&str], tile_size: usize) -> Vec<Vec<Vec<f32>>> {
        let mut bands: Vec<Vec<Vec<f32>>> = vec![];

        // load each band given by distinct .tif files
        for path in paths.iter() {
            let gcm = get_gdal_config_metadata(path, ContentType::input_data);
            let init_op = initialize_operator(gcm, tile_size).await;
            let buffer_proc = get_band_data(init_op).await;
            bands.push(buffer_proc);
        }
    ...
    async fn make_xg_data_no_labels(zipped_data: Vec<Vec<Vec<f32>>>) -> DMatrix {
        ...
        let byte_size_ax_0 = mem::size_of::<f32>() * strides_ax_0;
        let byte_size_ax_1 = mem::size_of::<f32>() * strides_ax_1;
    ...
    async fn generate_reservoir<'a>(
        ...
        for (tile_counter, tile) in zipped_data.iter().enumerate() {
            ...
            while i < (tile_counter + 1) * (tile_size * tile_size) {
                ...
                w = w * (uniform_rand.ln() / capacity as f32).exp();
    ...
    fn generate_uniform_rng(from: f32, to: f32) -> f32 {
        let step = Uniform::new(from, to);
        let mut rng = rand::thread_rng();
        let choice: f32 = step.sample(&mut rng);
        choice
    }

    // async fn predict(booster_model: Booster, z: &Vec<Vec<Vec<f64>>>) -> Result<Vec<f32>, XGBError> {
    //     let paths = [
    ...
    fn xg_test() {
        ...
        // get xgboost style matrices
        let mut test_data = DMatrix::from_col_major_f32(
